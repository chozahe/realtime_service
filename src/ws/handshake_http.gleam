...
import ws/handshake.{type HandshakeError, AuthenticationFailed, TokenExtractionFailed}
import ws/token.{InvalidAuthorizationHeader, MissingToken}

pub fn unauthorized_response(error: HandshakeError) -> http_response.Response(String) {
...
fn error_message(error: HandshakeError) -> #(String, String) {
  case error {
    ...
    AuthenticationFailed(MalformedToken) ->
    ...
    AuthenticationFailed(TokenExpired(_)) ->
      #("Token expired", "Refresh access token and reconnect")

    AuthenticationFailed(TokenNotYetValid(_)) ->
      #("Invalid token", "Token is not valid yet")

    AuthenticationFailed(UnsupportedTokenType) ->
      #("Invalid token type", "WebSocket handshake requires access token")

    AuthenticationFailed(InvalidUserId) ->
      #("Invalid token", "Subject claim is not a valid UUID")

    AuthenticationFailed(InvalidRoles) ->
      #("Invalid token", "Roles claim is invalid")

    AuthenticationFailed(InvalidIssuer) ->
      #("Invalid token", "Issuer claim does not match expected issuer")

    AuthenticationFailed(MissingClaim(claim_name)) ->
      #("Invalid token", "Missing required claim: " <> claim_name)

    AuthenticationFailed(NoMatchingKey) ->
      #("Invalid token", "No matching verification key found")
